<template>
  <div class="rpg-cave-page">
    ...
    <div class="party-row">
      <PartyCodeBar base-path="/rpg/cave" />
    </div>
    <main class="cave-body">
      ...
      <template v-else>
        <p class="hint">
          ใช้ปุ่มลูกศร หรือ W A S D เพื่อเดิน · E โต้ตอบ · Q ภารกิจ · I ถุงของ · L บันทึก DM — จุดสำรวจนอกเนื้อเรื่องหลัก มีของและมอนสเตอร์ให้ปราบเก็บเลเวล
        </p>
        <CaveCanvas :key="roomId" :class-id="classId" :room-id="roomId" />
      </template>
    </main>
  </div>
</template>
...
<script>
import CaveCanvas from '~/components/game/CaveCanvas.vue'
...
export default {
  name: 'RpgCave',
  components: { CaveCanvas, PartyCodeBar },
  ...
  computed: {
    ...
    roomId () {
      return 'cave-' + resolvePartyCode(this.$route, this.user)
    }
  }
}
</script>
