<template>
  <div class="rpg-forest-page">
    ...
    <div class="party-row">
      <PartyCodeBar base-path="/rpg/forest" />
    </div>
    <main class="forest-body">
      ...
      <template v-else>
        <p class="hint">
          ใช้ปุ่มลูกศร หรือ W A S D เพื่อเดิน · E โต้ตอบ · Q ภารกิจ · I ถุงของ · L บันทึก DM — ระวังภูตผีในป่าลึก
        </p>
        <ForestCanvas :key="roomId" :class-id="classId" :room-id="roomId" />
      </template>
    </main>
  </div>
</template>
...
export default {
  name: 'RpgForest',
  components: { ForestCanvas, PartyCodeBar },
  ...
  computed: {
    ...
    roomId () {
      return 'forest-' + resolvePartyCode(this.$route, this.user)
    }
  }
}
</script>
...
.rpg-bg {
  ...
  inset: 0;
  background-image:
    radial-gradient(circle at 20% 20%, rgba(90, 140, 70, 0.08), transparent 45%),
    radial-gradient(circle at 80% 70%, rgba(40, 70, 40, 0.15), transparent 45%);
  pointer-events: none;
  z-index: 0;
}
